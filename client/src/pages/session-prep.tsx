    ...
    return (
      <div className="container mx-auto p-6">
        <div className="animate-pulse space-y-6">
          <div className="h-8 bg-teal/30 rounded w-1/3"></div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-64 bg-teal/30 rounded-lg"></div>
            ))}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <p className="text-sepia">Unable to load session preparation data.</p>
          <Link href="/calendar">
    ...
    <div className="container mx-auto p-6 space-y-6" data-testid="session-prep-page">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <p className="text-sm font-medium text-sepia/80">Client</p>
              <p className="text-lg font-semibold">{client.name}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-sepia/80">Date & Time</p>
              <p className="text-lg">{sessionTime}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-sepia/80">Duration</p>
              <p className="text-lg">{session.duration} minutes</p>
            </div>
            <div>
              <p className="text-sm font-medium text-sepia/80">Type</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {recentNotes.length > 0 ? (
              recentNotes.map((note, index) => (
                <div key={note.id} className="space-y-2">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-xs text-sepia/80">
                      {formatInTimeZone(new Date(note.createdAt), 'America/New_York', 'MMM dd')}
                    </span>
                  </div>
                  <p className="text-sm text-sepia line-clamp-3">
              ...
              <p className="text-sm text-sepia/80">No recent progress notes available.</p>
            )}
          </CardContent>
        </Card>
            ...
            {caseConceptualization ? (
              <>
                <div>
                  ...
                  <p className="text-sm text-sepia">{caseConceptualization.presenting}</p>
                </div>

                <div>
                  <h4 className="font-medium text-sm mb-2">Predisposing Factors</h4>
                  <p className="text-sm text-sepia">{caseConceptualization.predisposing}</p>
                </div>

                <div>
                  <h4 className="font-medium text-sm mb-2">Protective Factors</h4>
                  <p className="text-sm text-sepia">{caseConceptualization.protective}</p>
                </div>
              </>
            ) : (
              <p className="text-sm text-sepia/80">No case conceptualization available.</p>
            )}
          </CardContent>
        </Card>
                  ...
                  <div>
                    ...
                    <ul className="text-sm space-y-1">
                      {treatmentPlan.goals.slice(0, 3).map((goal, index) => (
                        <li key={index} className="text-sepia">• {goal}</li>
                      ))}
                    </ul>
                  </div>
                )}
            ...
            {caseConceptualization && (
              <div>
                <h4 className="font-medium text-sm mb-2 flex items-center">
                  <AlertTriangle className="w-4 h-4 mr-1 text-teal" />
                  Perpetuating Factors
                </h4>
                <p className="text-sm text-sepia">{caseConceptualization.perpetuating}</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {activePrep && (
        <Card>
          ...
          <CardContent className="space-y-4 text-sm text-ink/90">
            {prepHistory.length > 0 && (
              <div className="flex items-center gap-2 text-sm">
                <label htmlFor="prep-history" className="text-sepia">Prep history</label>
                <select
                  id="prep-history"
                ...
                {prepHistory.length > 1 && (
                  <>
                    <label htmlFor="prep-compare" className="text-sepia">Compare to</label>
                    <select
                      id="prep-compare"
                      className="border rounded px-2 py-1"
                      value={compareIndex}
            ...
            <div>
              <h4 className="font-semibold text-ink">Where We Left Off</h4>
              <p className="text-sepia">{activePrep.where_we_left_off.session_ending_note}</p>
              <div className="mt-2">
            ...
            <Separator />
            <div>
              <h4 className="font-semibold text-ink">Homework Follow-Up</h4>
            ...
            <Separator />
            <div>
              <h4 className="font-semibold text-ink">Clinical Flags</h4>
            ...
            <Separator />
            <div>
              <h4 className="font-semibold text-ink">Suggested Openers</h4>
              <ul className="list-disc pl-5">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 lg:grid-cols-2 gap-4 text-sm">
            <div className="lg:col-span-2 flex items-center justify-between text-xs">
              <span className="text-sepia/80">Diff view</span>
            ...
            {(() => {
              ...
              return (
                <div className="lg:col-span-2 rounded border p-3 space-y-2">
                  <div className="text-xs text-sepia/80">Diff highlights</div>
            ...
            {showFullDiff && (
              <div className="lg:col-span-2 rounded border p-3 space-y-3 text-xs text-sepia">
                <div className="font-semibold text-ink">Inline Text Diff</div>
                {[
                  {
                  ...
                  <div key={index} className="space-y-1">
                    <div className="text-sepia/80">{item.label}</div>
                    <div className="flex flex-wrap gap-1">
            ...
            <div className="space-y-2">
              <h4 className="font-semibold text-ink">Selected Prep</h4>
              <div className="rounded border p-3 space-y-2">
                <div>
                  <div className="text-xs text-sepia/80">Themes</div>
                  <div>{activePrep.where_we_left_off.key_themes.join(", ")}</div>
                </div>
                <div>
                  <div className="text-xs text-sepia/80">Risk Level</div>
                  <div>{activePrep.clinical_flags.risk_level}</div>
                </div>
                <div>
                  <div className="text-xs text-sepia/80">Focus Suggestions</div>
            ...
            <div className="space-y-2">
              <h4 className="font-semibold text-ink">Comparison Prep</h4>
              <div className="rounded border p-3 space-y-2">
                <div>
                  <div className="text-xs text-sepia/80">Themes</div>

...
export default function BulkTranscripts() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      failed: { 
        ...
        text: 'Failed',
        className: 'bg-evergreen/20 text-white border-evergreen/30' 
      },
    };
  ...
  const getFileStatusIcon = (status: string, processingStatus?: string | null) => {
    switch (status) {
      ...
      case 'failed':
        return <AlertCircle className="h-4 w-4 text-evergreen" />;
      default:
  ...
  return (
    <div className="bulk-transcripts-page space-y-8" data-testid="bulk-transcripts-page">
      ...
      <div>
        ...
        <p className="mt-2 text-lg text-moss">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white border-moss/20">
            <CardContent className="p-6">
              <div className="flex items-center">
                <FileText className="h-8 w-8 text-french-blue mb-2" />
                <div className="ml-4">
                  <p className="text-sm font-medium text-moss">Total Files</p>
                  <p className="text-2xl font-bold text-evergreen">{stats.totalFiles}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-moss/20">
            <CardContent className="p-6">
              <div className="flex items-center">
                <CheckCircle className="h-8 w-8 text-sage mb-2" />
                <div className="ml-4">
                  <p className="text-sm font-medium text-moss">Processed</p>
                  <p className="text-2xl font-bold text-evergreen">{stats.processedFiles}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-moss/20">
            <CardContent className="p-6">
              <div className="flex items-center">
                <Eye className="h-8 w-8 text-french-blue mb-2" />
                <div className="ml-4">
                  <p className="text-sm font-medium text-moss">Need Review</p>
                  <p className="text-2xl font-bold text-evergreen">{stats.filesNeedingReview}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-moss/20">
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-moss">Total Batches</p>
                  <p className="text-2xl font-bold text-evergreen">{stats.totalBatches}</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Tabs defaultValue="upload" className="space-y-6">
        ...
        <TabsContent value="upload">
          <Card className="bg-white border-moss/20">
            <CardHeader>
              <CardTitle className="text-evergreen">Upload Transcript Batch</CardTitle>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="batch-name" className="text-evergreen">Batch Name</Label>
                <Input
                  id="batch-name"
                  ...
                  className="border-moss/30 focus:ring-sage focus:border-sage"
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="files" className="text-evergreen">Transcript Files</Label>
                <Input
                  id="files"
                  type="file"
                  multiple
              ...
              {selectedFiles && selectedFiles.length > 0 && (
                <div className="p-4 bg-ivory border border-moss/20 rounded-md">
                  <p className="text-sm font-medium text-evergreen">
                    Selected: {selectedFiles.length} files
                  </p>
                  <div className="mt-2 max-h-32 overflow-y-auto">
                    {Array.from(selectedFiles).map((file, index) => (
                      <p key={index} className="text-xs text-moss truncate">
              ...
              <Button
                ...
                className="w-full bg-sage hover:bg-sage/80 text-white"
                              ...
                              <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                  ...
                                  {(batch.failedFiles || 0) > 0 && (
                                    <div className="flex items-center space-x-2 text-sm">
                                      <AlertCircle className="h-4 w-4 text-evergreen" />
                                      <span className="text-evergreen">Failed: {batch.failedFiles}</span>
                                    </div>
                                  )}
                                </div>
        ...
        <TabsContent value="review">
          <Card className="bg-white border-moss/20">
            <CardHeader>
              <CardTitle className="text-evergreen">Manual Review Required</CardTitle>
              <CardDescription className="text-moss">
            ...
            <CardContent>
              {reviewLoading ? (
                <div className="text-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-sage mx-auto"></div>
                  <p className="mt-2 text-moss">Loading files...</p>
                </div>
                ...
                <div className="space-y-4">
                  {reviewFiles.map((file) => (
                    <div
                      key={file.id}
                      className="p-4 border border-french-blue/20 bg-french-blue/5 rounded-lg"
                      data-testid={`review-file-${file.id}`}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-semibold text-evergreen">{file.fileName}</h3>
                          <p className="text-sm text-moss">
                            Confidence: {((file.clientMatchConfidence || 0) * 100).toFixed(1)}%
                          </p>
                          {file.suggestedClientName && (
                            <p className="text-sm text-french-blue">
                        ...
                        <div className="flex flex-col space-y-2">
                          <select 
                            className="text-sm border border-moss/30 rounded px-2 py-1 focus:ring-sage focus:border-sage"
                          ...
                          <Input
                            type="date"
                            className="text-sm border-moss/30 focus:ring-sage focus:border-sage"
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-moss">All files have been processed successfully</p>
                </div>
              )}
            </CardContent>
          </Card>

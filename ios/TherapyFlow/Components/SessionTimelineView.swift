...
struct SessionTimelineView: View {
    ...
    private var clientFilterSection: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                TimelineFilterChip(title: "All Clients", isSelected: selectedClient == nil) {
                    selectedClient = nil
                }
                TimelineFilterChip(title: "Recent", isSelected: selectedClient == "recent") {
...
struct TimelineSession: Identifiable {
    ...
    enum SessionType {
        ...
        var color: Color {
            switch self {
            case .upcoming: return Color.theme.info
            case .completed: return Color.theme.success
            case .cancelled: return Color.theme.error
            }
        }

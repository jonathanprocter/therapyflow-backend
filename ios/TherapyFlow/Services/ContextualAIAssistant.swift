...
class ContextualAIAssistant: ObservableObject {
    ...
    private func askQuestionLocally(_ question: String, fullContext: FullApplicationContext?, historyStrings: [String]?) async throws -> AIAssistantResponse {
        ...
        guard let url = URL(string: "https://api.anthropic.com/v1/messages") else {
            throw NSError(domain: "ContextualAIAssistant", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid API URL configuration"])
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw NSError(domain: "ContextualAIAssistant", code: -2, userInfo: [NSLocalizedDescriptionKey: "Invalid HTTP response from AI service"])
        }

        guard (200...299).contains(httpResponse.statusCode) else {
            let errorBody = String(data: data, encoding: .utf8) ?? "No error details"
            print("[ContextualAIAssistant] AI API error: HTTP \(httpResponse.statusCode), Response: \(errorBody)")
            throw NSError(domain: "ContextualAIAssistant", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: "AI service error (HTTP \(httpResponse.statusCode))"])
        }
        ...
        guard !anthropicResponse.content.isEmpty else {
            throw NSError(domain: "ContextualAIAssistant", code: -3, userInfo: [NSLocalizedDescriptionKey: "Empty response from AI service"])
        }

        guard let textContent = anthropicResponse.content[0].text else {
            throw NSError(domain: "ContextualAIAssistant", code: -4, userInfo: [NSLocalizedDescriptionKey: "No text content in AI response"])
        }

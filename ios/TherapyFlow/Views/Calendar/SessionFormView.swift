...
struct SessionFormView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Notes (Optional)") {
                ...
                if notes.count > 9000 {
                    Text("\(notes.count)/10000 characters")
                        .font(.caption)
                        .foregroundColor(notes.count > 10000 ? .red : Color.theme.secondaryText)
                }
            }
    ...
    private var isValid: Bool {
        ...
        return selectedClientId != nil && !selectedClientId!.isEmpty
    }

    private var validationMessage: String? {
        if !isEditMode && selectedClientId == nil {
            return "Please select a client"
        }
        if notes.count > 10000 {
    ...
    private func save() {
        guard isValid else {
            if let message = validationMessage {
                self.error = NSError(domain: "SessionFormView", code: -1, userInfo: [NSLocalizedDescriptionKey: message])
            }
            return
        }
        ...
        Task {
            do {
                ...
                if let editSession = editSession {
                    ...
                    guard let clientId = selectedClientId, !clientId.isEmpty else {
                        await MainActor.run {
                            self.error = CreateSessionInput.ValidationError.emptyClientId
                            isLoading = false
                        }
                        return
                    }

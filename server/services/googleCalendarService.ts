...
export class GoogleCalendarService {
  ...
  /**
   * Set OAuth tokens directly (for iOS app integration)
   */
  setTokens(tokens: { access_token: string; refresh_token?: string; expiry_date?: number; token_type?: string }): void {
    try {
      this.oauth2Client.setCredentials({
  ...
  /**
   * Clear stored OAuth tokens (for disconnect)
   */
  clearTokens(): void {
    try {
      this.oauth2Client.setCredentials({});
